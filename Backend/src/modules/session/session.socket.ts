...
import type { ClientResponse, ServerResponse } from "./session.types"
import * as sessionController from './session.controller'
import { sessions, cleanupSession } from "./session"
...
export const handleInterview = (wss: WebSocketServer) => {
    wss.on('connection', (socket: WebSocket, req: IncomingMessage) => {
        ...
        socket.on('message', async (data) => {
            ...
            await sessionController.handleMessage(sessionId, message, decoded.userId,)
        })
